...
import { ENS_TEXT_KEYS, getArtistEnsRoot, getCollectionEns, getDropContractAddress } from "@/lib/constants";
import { writeEnsTextRecords } from "@/lib/ens";
import { saveCollection } from "@/lib/local-cache";
...
export function CreateFlow() {
  ...
  async function publish() {
    if (!address || !walletClient || !publicClient) {
      setError("Connect a Sepolia wallet before publishing ENS records.");
      return;
    }
    ...
    try {
      ...
      const record: CollectionRecord = {
        ...
        algorithmHash: bundle.packageHash,
        codeURI: walrus.uri,
        contract: getDropContractAddress(),
      ...
      await writeEnsTextRecords({
        ...
        records: {
          ...
          [ENS_TEXT_KEYS.algorithmHash]: bundle.packageHash,
          [ENS_TEXT_KEYS.codeURI]: walrus.uri,
          [ENS_TEXT_KEYS.contract]: getDropContractAddress(),
    ...
    <main className="mx-auto max-w-7xl px-5 py-10">
      ...
      <section className="grid gap-8 lg:grid-cols-[360px_1fr]">
        <aside className="space-y-5">
          ...
          >
            {publishing ? <Loader2 className="animate-spin" size={16} /> : <FileUp size={16} />}
            Publish to Walrus + ENS
          </button>
